...
class TrainingDataSet(Dataset):
    def __init__(self, sliding_dir, it_path, batch_size):
        ...
        f = open(it_path, 'rb')
        cs = pickle.load(f, encoding='bytes')  # 一个视频片段对应的那个几个查询？
        f.close()
        f = open("./video_allframes_info.pkl", 'rb')  # 视频长度？
        movie_length_info = pickle.load(f)
        f.close()
        self.clip_sentence_pairs = []
        for l in cs:
            clip_name = l[0].decode()
            sent_vecs = l[1]
            for sent_vec in sent_vecs:
                self.clip_sentence_pairs.append((clip_name, sent_vec))  # 片段名与查询对应
        ...
        for k in range(len(self.clip_sentence_pairs)):
            ...
            self.movie_clip_names[movie_name].append(k)  # 创建一个视频的集合，并把一个视频下的所有片段放在一起
        self.movie_names = list(movie_names_set)
        self.visual_feature_dim = 4096 * 3
        self.sent_vec_dim = 4800
        self.num_samples = len(self.clip_sentence_pairs)
        self.sliding_clip_path = sliding_dir #视觉特征目录
        ...
        for clip_name in sliding_clips_tmp:
            if clip_name.split(".")[2] == "npy": # 都保存在了.npy中
                movie_name = clip_name.split("_")[0] # 视频的名字
                for clip_sentence in self.clip_sentence_pairs: # 遍历所有剪辑-查询对
                    original_clip_name = clip_sentence[0] # 剪辑名
                    original_movie_name = original_clip_name.split("_")[0] # 剪辑对应的视频名
                    if original_movie_name == movie_name: # 如果当前npy的视频名和遍历出的视频名相同
                        start = int(clip_name.split("_")[1])
                        end = int(clip_name.split("_")[2].split(".")[0]) # npy中剪辑的开始和结束位置
                        o_start = int(original_clip_name.split("_")[1])
                        o_end = int(original_clip_name.split("_")[2].split(".")[0]) # 原剪辑的开始和结束
                        iou = calculate_IoU((start, end), (o_start, o_end)) # 计算两者的iou
                        if iou > 0.5: # 一般认为iou大于0.5是目标识别的不错的结果
                            nIoL = calculate_nIoL((o_start, o_end), (start, end))
                            if nIoL < 0.15:
                                movie_length = movie_length_info[movie_name.split(".")[0]]
                                start_offset = o_start - start
                                end_offset = o_end - end # 计算了偏移量
                                # 剪辑名 查询句子 视觉特征文件名 偏移量。
                                # 一个视觉特征会对应多个剪辑，只要他们是属于同一个视频而且iou和nIoL符合条件
    ...
    def __getitem__(self, index):
        clip_name = self.clip_sentence_pairs_iou[index][0] #剪辑名
        feat_path = self.sliding_clip_path + self.clip_sentence_pairs_iou[index][2] #对应的视觉特征的文件路径
        featmap = np.load(feat_path)
        # 读取上下文特征
        left_context_feat, right_context_feat = \
            self.get_context_window(self.clip_sentence_pairs_iou[index][2], self.context_num)
        image = np.hstack((left_context_feat, featmap, right_context_feat))
        sentence = self.clip_sentence_pairs_iou[index][1][:self.sent_vec_dim]
        p_offset = self.clip_sentence_pairs_iou[index][3]
        l_offset = self.clip_sentence_pairs_iou[index][4]
        offset = [p_offset, l_offset]
        return torch.tensor(image), torch.tensor(sentence), torch.tensor(offset)
    '''
    ...
    #             left_context_feat, right_context_feat = self.get_context_window(self.clip_sentence_pairs_iou[k][2],
    #                                                                             self.context_num)
...
class TestingDataSet(object):
    def __init__(self, img_dir, csv_path, batch_size):
        ...
        f = open(csv_path, 'rb')
        csv = pickle.load(f, encoding='bytes')
        f.close()
        self.clip_sentence_pairs = []
        for l in csv:
            clip_name = l[0].decode()
    ...
    def load_movie_slidingclip(self, movie_name, sample_num=1):
        movie_clip_sentences = []
        movie_clip_featmap = []
        clip_set = set()
        for k in range(len(self.clip_sentence_pairs)):
        ...
        return movie_clip_featmap, movie_clip_sentences # 所有剪辑及其对应的视觉特征， 所有剪辑及其对应的查询